...
#
my @ignoreDirs=( "application/indexes", "newsletter", "html/newsletter", "html/assets" );
...
if (! $doquiet) { print "\nDetermine s offset.\n"; }
if ($syncdirection eq "put") { clocksync($ftpc, "syncfile", $localdir . "/../"); }
...
sub clocksync {
    ...
    if(! -f $local_fn_file) {
      open(SF, ">$local_fn_file") or die "Cannot create $local_fn_file for time sync option";
      close(SF);
      $fndidexist=0;
    }
    -z $local_fn_file or
      die "File $local_fn_file for time sync must be empty.";
    my $putsyncok=1;
