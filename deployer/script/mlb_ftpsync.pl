...
#
# HARDCODED DIRECTORIES THAT WILL NOT BE DELETED ON THE REMOTE SITE
#
my @ignoreDirs = (
	"html/assets/videos",  "html/images/parks/featured",
	"application/indexes", "newsletter",
	"html/newsletter",     "html/assets"
);
...
for $curopt ( @cfgfoptions, @cloptions ) {
	if ( $curopt =~ /^-[a-zA-Z]/ ) {
		...
		for ( $i = 1 ; $i < length($curopt) ; $i++ ) {
			...
			if ( $curoptchar =~ /[cC]/ ) { $docheckfirst = 1; }
				...
				print "ERROR: Unknown option: \"-" . $curoptchar . "\"\n";
				$returncode += 1;
			}
		}
	}
		...
		/^ftp:\/\/(([^@\/\\\:]+)(:([^@\/\\\:]+))?@)?([a-zA-Z01-9\.\-]+)\/(.*)/ )
	{
		...
		elsif ( $fname eq "timeout" ) {
			if ( $fvalue > 0 ) { $ftptimeout = $fvalue; }
		}
...
if ( $ftpserver eq "" ) {
	print "ERROR: No FTP server given.\n";
	$returncode += 1;
}
if ( $ftpdir eq "" ) {
	print "ERROR: No FTP directory given.\n";
	$returncode += 1;
}
if ( $ftpuser eq "" ) { print "ERROR: No FTP user given.\n"; $returncode += 1; }
if ( $ftppasswd eq "" ) {
	print "ERROR: No FTP password given.\n";
	$returncode += 1;
}
if ( $returncode > 0 ) {
	die
"Aborting due to missing or wrong options! Call ftpsync -? for more information.\n";
}
...
$ftpc->login( $ftpuser, $ftppasswd )
  || die "Could not login to $ftpserver as $ftpuser\n";
...
{
	if   ( $ftpdefdir eq "/" ) { $ftpdir = $ftpdefdir . $ftpdir; }
	else                       { $ftpdir = $ftpdefdir . "/" . $ftpdir; }
	if ( !$doquiet ) { print "Absolute remote directory is $ftpdir\n"; }
}
...
if ( $syncdirection eq "put" ) {
	clocksync( $ftpc, "syncfile", $localdir . "/../" );
}
...
sub buildlocaltree() {
	if ($doflat) {
		...
		find( { wanted => \&noticelocalfile, follow_fast => $followsymlinks },
			$localdir . "/" );
	}
	...
	sub noticelocalfile {
		...
		if ( -d $_ ) {
			...
			if ( !$doquiet ) {
				print "Ignoring file of unknown type: "
				  . $File::Find::name . "\n";
			}
		}
	...
	if ($dodebug) {
		...
		my $curlocaldir = "";
		foreach $curlocaldir ( keys(%localdirs) ) { print $curlocaldir. "/\n"; }
...
sub buildremotetree() {
	...
	foreach $curlsline ( parse_dir( \@rfl ) ) {
		...
		if ($cftype) {
			...
			{    # link, rest of string = linkto
				...
				if ( $remotefiledates{$curnewrf} le 0 ) {
					die "Timeout detecting modification time of $curnewrf\n";
				}
				$remotefilesizes{$curnewrf} = $cfsize;
				if ( $remotefilesizes{$curnewrf} lt 0 ) {
					die "Timeout detecting size of $curnewrf\n";
				}
	...
	foreach $currecurseddir (@currecursedirs) {
		...
		foreach $ignoreDir (@ignoreDirs) {
			if ( $curremotesubdir =~ m/^$ignoreDir/ ) {
				print "\nNot syncing remote dir. $curremotesubdir\n";
				$skip = 1;
				last;
			}
		}
		...
		if ( $ftpdir eq "/" ) { $curcwddir = $ftpdir . $curremotesubdir; }
		else                  { $curcwddir = $ftpdir . "/" . $curremotesubdir; }
...
sub clocksync {
	my $conn          = shift @_;
	my $fn            = shift @_;
	my $local_fn_dir  = shift @_;
	my $local_fn_file = $local_fn_dir . $fn;
	my $fndidexist    = 1;

	if ( !-f $local_fn_file ) {
		open( SF, ">$local_fn_file" )
		  or die "Cannot create $local_fn_file for time sync option";
		close(SF);
		$fndidexist = 0;
	}
	-z $local_fn_file
	  or die "File $local_fn_file for time sync must be empty.";
	my $putsyncok = 1;
	$conn->put( $local_fn_file, $fn ) or $putsyncok = 0;
	if ( !$putsyncok ) {
		unlink($local_fn_file);    # cleanup!
		die "Cannot send timesync file $local_fn_file";
	}

	my $now_here1 = time();
	my $now_there = $conn->mdtm($fn)
	  or die "Cannot get write time of timesync file $fn";
	my $now_here2 = time();
	...
	unlink($local_fn_file) unless $fndidexist;
}

sub dosync() {
	chdir $localdir
	  || die "Could not change to local base directory $localdir\n";
	if ( $syncdirection eq "put" ) {
			...
			keys(%localdirs) )
		{
			...
			my $ignoreDir;
			foreach $ignoreDir (@ignoreDirs) {
				if ( $curlocaldir =~ m/^$ignoreDir/ ) {
					print "Not creating remote subdirectory $curlocaldir\n";
					$skip = 1;
					last;
				}
			}
			...
			if ( !exists $remotedirs{$curlocaldir} ) {
				...
				if ( $skip == 0 && $ftpc->mkdir($curlocaldir) ne $curlocaldir )
				{
					...
					$ftpc->quot(
						'SITE',
						sprintf( 'CHMOD %04o %s',
							( lstat $curlocaldir )[2] & 07777, $curlocaldir )
					);
				}
			}
		}
...
# copy files missing or too old at the target, synchronize timestamp _after_ copying
		...
		my $curlocalfile;
		foreach $curlocalfile ( sort { return length($b) <=> length($a); }
			keys(%localfiledates) )
		{
			my $skip = 0;
			my $ignoreDir;
			foreach $ignoreDir (@ignoreDirs) {
				if ( $curlocalfile =~ m/^$ignoreDir/i ) {
					print "\nNot uploading file. $curlocalfile\n";
					$skip = 1;
					last;
				}
			}
			...
			if ( !exists $remotefiledates{$curlocalfile} ) {
				...
				$infotext  = "New: "
				  . $curlocalfile . " ("
				  . $localfilesizes{$curlocalfile}
				  . " bytes)\n";
				if ($doinfoonly) { print $infotext; next; }
				elsif ($doverbose)  { print $infotext; }
				elsif ( !$doquiet ) { print "n"; }
			}
			elsif ( $remotefiledates{$curlocalfile} <
				$localfiledates{$curlocalfile} )
			{
				$dorefresh = 1;
				$infotext =
				    "Newer: "
				  ...
				  . $localfiledates{$curlocalfile}
				  . " versus "
				  . $remotefiledates{$curlocalfile} . ")\n";
				if ($doinfoonly) { print $infotext; next; }
				if ($doverbose) { print $infotext; }
				elsif ( !$doquiet ) { print "u"; }
			}
			elsif ( $remotefilesizes{$curlocalfile} !=
				$localfilesizes{$curlocalfile} )
			{
				$dorefresh = 1;
				$infotext =
				    "Changed (different sized): "
				  . $curlocalfile . " ("
				  . $localfilesizes{$curlocalfile}
				  . "  versus "
				  . $remotefilesizes{$curlocalfile}
				  . " bytes)\n";
				if ($doinfoonly) { print $infotext; next; }
				if ($doverbose) { print $infotext; }
				elsif ( !$doquiet ) { print "u"; }
			...
			if ( !$dorefresh ) { next; }
			if ($dodebug) {
				print "Really PUTting file " . $curlocalfile . "\n";
			}
			if (   $skip == 0
				&& $ftpc->put( $curlocalfile, $curlocalfile ) ne $curlocalfile )
			{
				...
				and ( $retries-- > 0 ) )
			{
				...
				if (   $skip == 0
					&& $ftpc->put( $curlocalfile, $curlocalfile ) ne
					$curlocalfile )
				{
		...
		if ( !$nodelete ) {
			...
			foreach $curremotefile ( keys(%remotefiledates) ) {
				...
				my $ignoreDir;
				foreach $ignoreDir (@ignoreDirs) {
					if ( $curremotefile =~ m/^$ignoreDir/i ) {
						print "\nNot deleting remote file. $curremotefile\n";
						$skip = 1;
						last;
					}
				}
				...
				if ( not exists $localfiledates{$curremotefile} ) {
					...
					if ( $ftpc->delete($curremotefile) ne 1 ) {
						die "Could not delete remote file $curremotefile\n";
					}
				}
			}
			...
			my $curremotedir;
			foreach $curremotedir ( sort { return length($b) <=> length($a); }
				keys(%remotedirs) )
			{
				my $skip = 0;
				my $ignoreDir;
				foreach $ignoreDir (@ignoreDirs) {
					if ( $curremotedir =~ m/^$ignoreDir/i ) {
						print "\nNot deleting remote directory $curremotedir\n";
						$skip = 1;
						last;
					}
				}
				...
				if ( !exists $localdirs{$curremotedir} ) {
					...
					if ( $ftpc->rmdir($curremotedir) ne 1 ) {
						die
"Could not remove remote subdirectory $curremotedir\n";
					}
				}
			}
		}
		...
		my $curremotedir;
		foreach $curremotedir ( sort { return length($a) <=> length($b); }
			keys(%remotedirs) )
		{
			if ( !exists $localdirs{$curremotedir} ) {
				if ($doinfoonly) { print $curremotedir. "\n"; next; }
				...
				mkdir($curremotedir)
				  || die "Could not create local subdirectory $curremotedir\n";
			}
		}
...
# copy files missing or too old at the target, synchronize timestamp _after_ copying
		...
		my $curremotefile;
		foreach $curremotefile ( sort { return length($b) <=> length($a); }
			keys(%remotefiledates) )
		{
			my $dorefresh = 0;
			if ( !exists $localfiledates{$curremotefile} ) {
				...
				$infotext  = "New: "
				  . $curremotefile . " ("
				  . $remotefilesizes{$curremotefile}
				  . " bytes)\n";
				if ($doinfoonly) { print $infotext; next; }
				if ($doverbose) { print $infotext; }
				elsif ( !$doquiet ) { print "n"; }
			}
			elsif ( $remotefiledates{$curremotefile} >
				$localfiledates{$curremotefile} )
			{
				$dorefresh = 1;
				$infotext =
				    "Newer: "
			...
			elsif ( $remotefilesizes{$curremotefile} !=
				$localfilesizes{$curremotefile} )
			{
				$dorefresh = 1;
				$infotext =
				    "Changed (different sized): "
			...
			if ( !$dorefresh ) { next; }
			if ($dodebug) {
				print "Really GETting file " . $curremotefile . "\n";
			}
			...
			my $retries = 3;
			while (
				( $ftpc->size($curremotefile) != ( lstat $curremotefile )[7] )
				and ( $retries-- > 0 ) )
			{
		...
		if ( !$nodelete ) {
				...
				keys(%localfiledates) )
			{
				if ( not exists $remotefiledates{$curlocalfile} ) {
					...
					if ( unlink($curlocalfile) ne 1 ) {
						die "Could not remove local file $curlocalfile\n";
					}
				}
			}
			...
			foreach $curlocaldir ( keys(%localdirs) ) {
				if ( !exists $remotedirs{$curlocaldir} ) {
					...
					rmdir($curlocaldir)
					  || die
					  "Could not remove local subdirectory $curlocaldir\n";
				}
			}
		}
	}
